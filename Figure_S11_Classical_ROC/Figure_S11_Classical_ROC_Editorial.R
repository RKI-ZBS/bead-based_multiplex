...
# - Determine threshold for ratios of homologue antigens
...
dataInWideAntigens <- 
  ...
  mutate(panel_Seropos = if_else(panel_ATI %in% c("MPXV SPox", "MPXV", "MVA", "MVA SPox"), "MPXV/MVA", "Pre"),
         ratio_A29_A27 = dataIn_A29L/dataIn_A27L,
         ratio_M1_L1 = dataIn_M1R/dataIn_L1R,
         ratio_A35_A33 = dataIn_A35R/dataIn_A33R,
         ratio_E8_D8 = dataIn_E8L/dataIn_D8L,
         ratio_B6_B5 = dataIn_B6R/dataIn_B5R)
...
dataInWideATI_N <- 
  ...
  mutate(ratio_A29_A27 = dataIn_A29L/dataIn_A27L,
         ratio_M1_L1 = dataIn_M1R/dataIn_L1R,
         ratio_A35_A33 = dataIn_A35R/dataIn_A33R,
         ratio_E8_D8 = dataIn_E8L/dataIn_D8L,
         ratio_B6_B5 = dataIn_B6R/dataIn_B5R)
...
source_data_FigS11ab <- 
  dataInWideAntigens %>% 
  select(sampleID_metadata, c(panel:Age_group), panel_ATI, c(dataIn_D8L:dataIn_Delta))

source_data_FigS11cd <- 
  dataInWideATI_N %>% 
  select(sampleID_metadata, c(panel:Age_group), panel_ATI_N, c(dataIn_D8L:dataIn_Delta), 
         c(ratio_A29_A27:ratio_B6_B5))

export(file = "output/Source_Data_FigS11.xlsx",
       list(FigS11_ab = source_data_FigS11ab,
            FigS11_cd = source_data_FigS11cd))
...
roc_ratioA29A27_Infected <-
  roc(factor(as.character(dataInWideATI_N$panel_ATI_N), levels = c("MVA", "MPXV"),
             ordered = TRUE),dataInWideATI_N$ratio_A29_A27)

roc_ratioM1L1_Infected <-
  roc(factor(as.character(dataInWideATI_N$panel_ATI_N), levels = c("MVA", "MPXV"),
             ordered = TRUE),dataInWideATI_N$ratio_M1_L1)

roc_ratioE8D8_Infected <-
  roc(factor(as.character(dataInWideATI_N$panel_ATI_N), levels = c("MVA", "MPXV"),
             ordered = TRUE),dataInWideATI_N$ratio_E8_D8)

roc_ratioA35A33_Infected <-
  roc(factor(as.character(dataInWideATI_N$panel_ATI_N), levels = c("MVA", "MPXV"),
             ordered = TRUE),dataInWideATI_N$ratio_A35_A33)

roc_ratioB6B5_Infected <-
  roc(factor(as.character(dataInWideATI_N$panel_ATI_N), levels = c("MVA", "MPXV"),
             ordered = TRUE),dataInWideATI_N$ratio_B6_B5)
...
export(file = "output/2025-08-26_Populations_based_Cutoff.xlsx", paramDataFrame_transpose_sep)
...
plotrocRatios <- 
  plotROCfunction(list("A29/A27" = roc_ratioA29A27_Infected,
                       "M1/L1" = roc_ratioM1L1_Infected,
                       "E8/D8" = roc_ratioE8D8_Infected,
                       "A35/A33" = roc_ratioA35A33_Infected,
                       "B6/B5" = roc_ratioB6B5_Infected), "IgG")

plotRocCombined <-
  ggarrange(plotrocAntigensGood, plotrocAntigensBad, plotrocMPXVMVA, plotrocRatios,
...
ggsave(filename = "output/Fig_S11.png", plotFigS11 ,
