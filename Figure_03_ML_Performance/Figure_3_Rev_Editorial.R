...
# Changes made to the Editorial revisions
# - Export underlying data as Source Data
####
...
plotPerformance <- 
  ...
  filter(!(seropanel %in% c("all acute", "all epi", "epi all", "acute all"))) %>% 
...
####
# Generate Source Data file: plot performance
source_data_Fig3a <-
  statisticalDataCombined %>% 
  filter(parameter %in% c("mean", "stdev")) %>%
  pivot_longer(cols = c(accuracy, precision, recall, fscore), names_to = "score", values_to = "values") %>%
...
pdf("output/Fig3_all_xgboost_lda.pdf", width=12, height=6)
...
pdf("output/Fig3_all_rf.pdf", width=6, height=6)
...
####
# Export source data
export(file = "output/Source_Data_Fig3ab.xlsx", list(Fig3a = source_data_Fig3a,
                                                     Fig3b = source_data_Fig3b))
