...
source_data_FigS1_corr_ELISA_IgG <-
  dataInputCorrMultiplexIgG
...
####
# Generate Source Data Files
source_data_FigS1_corr_IFA_IgG <-
  dataInputCorIFAIgG
...
####
# Generate Source Data Files
source_data_FigS1_ELISA_IgG <-
  dataDeltaIgG %>% 
  select(-remarks, -remarks_ELISA)
  ...
  mutate(NT_titer_bin = case_when(NT_titer == 14 ~ paste0(intToUtf8(8804), "14"),
         ...
         NT_titer_bin = factor(NT_titer_bin,
                               levels = c(paste0(intToUtf8(8804), "14"),
                                          ...
                                          paste0(intToUtf8(8804), "75"),
                                          paste0(intToUtf8(8804), "100")), ordered = TRUE)) %>% 
  select(sampleID_metadata, isotype, analyte, dataIn, NT_titer, NT_titer_bin)

source_data_FigS1_NT_IgM <-
  dataInputNT %>% 
  dplyr::filter(dilution_assay == 100) %>% 
  dplyr::filter(isotype == "IgM") %>% 
  dplyr::filter(assaytype == "Multiplex") %>% 
...
# Figure S2: Plot IgG and IgM multiplex antigens in comparison to ELISA
...
source_data_FigS2 <-
  dataInputELISAMultiplex %>% 
  filter(isotype %in% c("IgG", "IgM")) %>% 
  select(sampleID_metadata, isotype, analyte, dataIn_ELISA, dataIn)
...
export(file = "output/Source_Data_FigS1_S2.xlsx",
       list(FigS1_ELISA_IgG = source_data_FigS1_ELISA_IgG,
            FigS1_ELISA_IgM = source_data_FigS1_ELISA_IgM,
            FigS1_IFA_IgG = source_data_FigS1_IFA_IgG,
            FigS1_IFA_IgM = source_data_FigS1_IFA_IgM,
            FigS1_NT_IgG = source_data_FigS1_NT_IgG,
            FigS1_NT_IgM = source_data_FigS1_NT_IgM,
            FigS1_Corr_ELISA_IgG = source_data_FigS1_corr_ELISA_IgG,
            FigS1_Corr_IFA_IgG = source_data_FigS1_corr_IFA_IgG,
            FigS1_Corr_NT_IgG = source_data_FigS1_corr_NT_IgG))
