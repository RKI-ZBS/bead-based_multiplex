...
# Changes to version Editorial
# - Export Source Data files for Fig. 4 and Fig. S10
####
...
####
# Generate Source Data files for Fig. S10
source_data_FigS10_LDA <-
  ensemblePrediction %>%
  mutate(serostatus.delta = factor(serostatus.delta, levels = c(0,1),
                                   labels = c("negative", "positive"))) %>% 
  pivot_longer(cols = c(LDA_Pre, LDA_MVA, LDA_MPXV), names_to = "pred",
...
export(file = "output/Source_Data_FigS10.xlsx",
       list(FigS10_LDA = source_data_FigS10_LDA,
            FigS10_GBC = source_data_FigS10_GBC,
            FigS10_RF = source_data_FigS10_RF))
...
####
# Export Source Data file for Fig.4
source_data_Fig4a <- 
  heatmap_input_IgG %>% 
  select(c(panel:Delta,
           serostatus.delta,
           n_MVA_vacc,
           LDA_MVA,
           GBC_MVA,
...
source_data_Fig4b <-
  conf_mat_GBC_all$`Confusion Matrix`[[1]] %>% 
  mutate(algorithm = "GBC") %>% 
  rbind(
    conf_mat_LDA_all$`Confusion Matrix`[[1]] %>% 
      mutate(algorithm = "LDA")) %>% 
  rbind(
    conf_mat_RF_all$`Confusion Matrix`[[1]] %>% 
      mutate(algorithm = "RF"))

export(file = "output/Source_Data_Fig4.xlsx", list(Fig4a = source_data_Fig4a,
                                                   Fig4b = source_data_Fig4b))

# Export source data file for table 3: part validation panel
source_data_Table_3_validation <- 
  ensemblePrediction %>% 
  mutate(cohort = "combined") %>% 
  select(cohort, sampleID_metadata = sampleID_meta, real, filter_mean_LDA_confidence = mean_mean_conf,
         filter_LDA_ensemble_pred_frac = LDA_pred_ensemble,
         filter_RF_ensemble_pred_frac = RF_pred_ensemble,
         filter_GBC_ensemble_pred_frac = GBC_pred_ensemble,
         pred_serostatus = max_col,
         pred_LDA = LDA_max_col,
         pred_GBC = GBC_max_col,
         pred_RF = RF_max_col)

export(file = "output/Source_Data_Table3_validation.xlsx", list(Table3_validataion = source_data_Table_3_validation))
