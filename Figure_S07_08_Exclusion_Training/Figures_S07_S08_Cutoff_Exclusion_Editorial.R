####
# Generate Figures S07 and S08
# Daniel Stern RKI 
# 2025/03/05
# Version: Editorial
# Last changes: 2025/11/03
# Changes made to previous version:
# - Export of Source Data Files
####
...
load("input/dataClustering_post.Rdata")
...
source_data_FigS7a <- 
  ...
  filter(dataIn > 0) %>% 
  filter(isotype == "IgG") %>% 
  select(sampleID_metadata, panel_ATI, panel_detail, Age_group, analyte, isotype, dataIn, ratioAG_seropositive) %>% 
  unique()
...
plotRatiosYoung <-
  ...
  facet_wrap("analyte") +
  geom_jitter(aes(color = ratioAG_seropositive), width = 0.1, alpha = 0.5) +
  scale_y_continuous(name = "Ratio MPXV/VACV", limits = c(0.2, 2.5)) +
  scale_x_discrete(name = "") +
  scale_color_viridis_c(name = "Ratio seropositivity") +
  stat_compare_means(comparisons = list(c("Pre acute", "Pre epi"), c("MVA acute", "MVA epi"), c("MPXV acute", "MPXV epi")),label = "p.signif",
                     method = "wilcox.test") +
...
plotCombinedyoung <- 
  ggarrange(plotYoungAntigens,
            plotRatiosYoung,
            ncol = 2, align = "hv",
            common.legend = TRUE, labels = "auto")

ggsave(filename ="output/SFig_plotCombinedyoung.png", plotCombinedyoung, width = 10, height = 8, 
       dpi = 600)
...
source_data_FigS8a <-
  dataInWideAntigens %>% 
  select(sampleID_metadata, c(panel:Age_group), panel_ATI, c(dataIn_D8L:dataIn_Delta))

source_data_FigS8a_ATI_N <-
  dataInWideATI_N %>% 
  select(sampleID_metadata, c(panel:Age_group), panel_ATI_N, c(dataIn_D8L:dataIn_Delta))
...
source_data_FigS8b <-
  dataInWideRatios %>% 
  select(sampleID_metadata, c(panel:Age_group), panel_ATI, c(ratio_A29L_A27L:ratio_B6R_B5R))
...
export(file = "output/2024-06-19_Populations_based_Cutoff.xlsx", paramDataFrame_transpose_sep)
# Determine threshold 
threshold <-
...
threshold_spec <- data.frame(antigene = c("ATI-N", "E8L", "Delta"),
                             CI = c("specific", "specific", "specific"),
                             threshold = c(min(roc_ATI_N$thresholds[roc_ATI_N$specificities == 1]),
                                           min(roc_E8L$thresholds[roc_E8L$specificities == 1]),
                                           min(roc_Delta$thresholds[roc_Delta$specificities == 1])))
max(roc_ATI_N$sensitivities[roc_ATI_N$specificities > 0.95])

threshold_spec_95 <- data.frame(antigene = c("ATI-N", "E8L", "Delta"),
                                CI = c("specific", "specific", "specific"),
                                threshold = c(min(roc_ATI_N$thresholds[roc_ATI_N$specificities > 0.95]),
                                              min(roc_E8L$thresholds[roc_E8L$specificities > 0.95]),
                                              min(roc_Delta$thresholds[roc_Delta$specificities > 0.95])))

threshold_population <- 
  threshold #%>% 
#filter(antigene %in% c("ATI-N", "E8L", "Delta")) %>% 
#rbind(threshold_spec)

# Export results of threshold calculation for import in determination of specificities
save(threshold_population, file = "output/treshold_population.RData")
...
predictSPox <- 
  ...
  mutate(SPox_Pre_prediction = if_else(dataIn > threshold, "positive", "negative"),
         SPox_Pre_prediction_stringent = case_when(analyte == "ATI-N" & dataIn > 
         ...
         SPox_Pre_prediction_stringent_95 = case_when(analyte == "ATI-N" & dataIn > 
                                                        min(roc_ATI_N$thresholds[roc_ATI_N$specificities > 0.95]) ~ "positive",
                                                      analyte == "ATI-N" & dataIn <= 
                                                        min(roc_ATI_N$thresholds[roc_ATI_N$specificities > 0.95]) ~ "negative",
                                                      
                                                      analyte == "A33R" & dataIn > 
                                                        min(roc_A33R$thresholds[roc_A33R$specificities > 0.95]) ~ "positive",
                                                      analyte == "A33R" & dataIn <= 
                                                        min(roc_A33R$thresholds[roc_A33R$specificities > 0.95]) ~ "negative",
                                                      
                                                      analyte == "Delta" & dataIn > 
                                                        min(roc_Delta$thresholds[roc_Delta$specificities > 0.95]) ~ "positive",
                                                      analyte == "Delta" & dataIn <= 
                                                        min(roc_Delta$thresholds[roc_Delta$specificities > 0.95]) ~ "negative",
                                                      
                                                      analyte == "A35R" & dataIn > 
                                                        min(roc_A35R$thresholds[roc_A35R$specificities > 0.95]) ~ "positive",
                                                      analyte == "A35R" & dataIn <= 
                                                        min(roc_A35R$thresholds[roc_A35R$specificities > 0.95]) ~ "negative",
                                                      
                                                      analyte == "B5R" & dataIn > 
                                                        min(roc_B5R$thresholds[roc_B5R$specificities > 0.95]) ~ "positive",
                                                      analyte == "B5R" & dataIn <= 
                                                        min(roc_B5R$thresholds[roc_B5R$specificities > 0.95]) ~ "negative",
                                                      
                                                      analyte == "B6R" & dataIn > 
                                                        min(roc_B6R$thresholds[roc_B6R$specificities > 0.95]) ~ "positive",
                                                      analyte == "B6R" & dataIn <= 
                                                        min(roc_B6R$thresholds[roc_B6R$specificities > 0.95]) ~ "negative",
                                                      
                                                      analyte == "D8L" & dataIn > 
                                                        min(roc_D8L$thresholds[roc_D8L$specificities > 0.95]) ~ "positive",
                                                      analyte == "D8L" & dataIn <= 
                                                        min(roc_D8L$thresholds[roc_D8L$specificities > 0.95]) ~ "negative",
                                                      TRUE ~ SPox_Pre_prediction))



exportSpoxPrediction <-
  predictSPox %>% 
  filter(grepl("SPox", sampleID_metadata)) %>% 
  select(panel, panel_detail, Age_group, sampleID_metadata, PIN, 
         isotype, analyte, data, dataIn, serostatus_cat, SPox_Pre_prediction, SPox_Pre_prediction_stringent, SPox_Pre_prediction_stringent_95)

export(file = "output/2024-06-19_Spox_Serostatus_population_cutoff.xlsx", exportSpoxPrediction)
...
SFig_plotRoc <-
  ggarrange(plotrocAntigens, plotrocRatios, ncol = 2, align = "hv",
            labels = "auto")

ggsave(filename = "output/SFig_plotRoc.png", SFig_plotRoc, width = 12, height = 6, dpi = 600)
...
export(file = "output/Source_Data_FigS7_S8.xlsx",
       list(FigS7a = source_data_FigS7a,
            FigS7b = source_data_FigS7b,
            FigS8a = source_data_FigS8a,
            FigS8a_ATI_N = source_data_FigS8a_ATI_N,
            FigS8b = source_data_FigS8b))
