...
# - Include calculation of mean sensitivities and specificities using bootstrap analysis
# Changes to version 4.0
# - Harmonize filtering with regards to the ensemble prediction
# Changes to version 5.0
# - Export of source data for plotting of figures
# - Export of source data for table 3 
####
...
sampleIDs_ML_input <-
  unique(ensembleCombined$sampleID_metadata)
save(file = "output/sampleID.Rdata", sampleIDs_ML_input)
...
####
# Export source data file for Fig 3c
source_data_Fig3c <- 
...
export(file = "output/Source_Data_Fig3c.xlsx", list(Fig3c = source_data_Fig3c))

# Export source data file for table 3: part combined all all panel
source_data_Table_3_combined <- 
  ensembleCombined %>% 
  mutate(cohort = "combined") %>% 
  select(cohort, sampleID_metadata, real, filter_mean_LDA_confidence = mean_mean_conf,
...
export(file = "output/Source_Data_Table3.xlsx", list(Table3_combined = source_data_Table_3_combined))
